...
mysqli_select_db($con1,"tmc");

$con2 = mysqli_connect("localhost","root");
mysqli_select_db($con2,"v2");
...
define("FOLDER","1");
define("SOURCE","t1.");
define("FACILITY_CD","11003");

mkdir("v1/bloodproc/1");
mkdir("v1/bloodproc/2");

$conflicts = getConflicts('bloodproc','bloodproc_no',"where t1.facility_cd = '".FACILITY_CD."'");

foreach($conflicts as $i => $r){
    $file = fopen("v1/bloodproc/".FOLDER."/".$i.".txt","w");
...
function fieldsRemoveExclusion($table,$prefix){
    ...
    $query = "SELECT GROUP_CONCAT(COLUMN_NAME) as 'columns' FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '$table' AND TABLE_SCHEMA = 'tmc'";
